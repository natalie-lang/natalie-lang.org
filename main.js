anychart.onDocumentReady(()=> {
  fetch('/specs/stats.json')
    ...
    .then(latest => {
      document.getElementById('successful-tests-counter').innerText = latest["Successful Examples"]
      document.getElementById('failing-tests-counter').innerText = latest["Failed Examples"]
      document.getElementById('erroring-tests-counter').innerText = latest["Errored Examples"]
      document.getElementById('not-compiling-tests-counter').innerText = latest["Compile Errors"]
      document.getElementById('crashing-tests-counter').innerText = latest["Crashes"]
      document.getElementById('timeout-tests-counter').innerText = latest["Timeouts"]
    })
    .catch(error => {
      console.log(error)
      document.getElementById("is-natalie-ruby-yet-stats-panel").style = "display: none;"
    })
});
